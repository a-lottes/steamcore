...
`kMaxCases`) sits well clear of its cap after this feature's addition, the
largest single batch in the project's history: `make test-all` finishes at
**255 passed, 0 failed** (T17, 2026-09-07; the 255th case is the review-F3
threshold-failsafe regression test added in `/peer-review` fix-mode) — under half the registry's
capacity, so the cap named in plan.md's R8 risk remains a documented,
watched limit rather than something this feature came close to hitting.
...
| `make view` | One command from a clean checkout to five PNGs: `build/pattern.png` (the rendering-core fixture), `build/text_pattern.png` (the text-rendering fixture — all 43 defined characters, permuted), `build/title_screen.png` (start-screen's `READY` title screen), `build/galactic_invasion_pattern.png` (a mid-round `PLAYING` frame — the full 18-enemy formation, the player ship, a shot in flight, both HUD strings) and `build/galactic_invasion_win_pattern.png` (the `YOU WIN` end screen, galactic-invasion T12). Runs the real C++ test suite first (so every fixture reflects the *current* source tree, not a stale committed one, and none of the uncommitted dumps exist until that run writes them), then decodes all five. `VIEWER_PNG`/`TEXT_VIEWER_PNG`/`TITLE_VIEWER_PNG`/`GAME_VIEWER_PNG`/`GAME_WIN_VIEWER_PNG` are overridable. |
